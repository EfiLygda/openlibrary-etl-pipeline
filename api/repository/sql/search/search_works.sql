...
-- Aggregating the authors for each work in list of json objects with
-- each object having the author key and name
authors_per_work AS (
    SELECT
        w.work_key,
        json_agg(
...
edition_base AS (
    ...
    FROM
        catalog.editions e
        LEFT JOIN catalog.editions_details ed
        ON e.edition_key = ed.edition_key
        LEFT JOIN catalog.editions_publishing ep
        ON e.edition_key = ep.edition_key
)
------------------------------------------------------------------------------------------

------------------------------------------------------------------------------------------
-- Building final results by adding work key and title and sorting
-- by descending relevance score and limiting or offsetting when needed
SELECT
    w.work_key,
    w.title,
    apw.authors
FROM
    catalog.works AS w
    INNER JOIN text_relevance_scores AS ts
